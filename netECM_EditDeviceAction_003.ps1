...
$PilotUpdateDeviceCol = "DSPR $DeviceType OS and Office Patches Pilot"
$PilotIPUDeviceCol = "DSPR $DeviceType OS IPU Pilot"
$SWProfileAssigExcl = $PilotUpdateDeviceCol,$PilotIPUDeviceCol
...
$PilotUpdateDeviceCol = Get-WmiObject -ComputerName $sccmserver -Namespace "Root\SMS\Site_$SiteCode" -Query "select * from SMS_Collection Where SMS_Collection.Name='$PilotUpdateDeviceCol'"
$PilotUpdateDeviceCol.Get()
$PilotUpdateColExists = $false
if ($PilotUpdateDevice -eq "True"){
    # Assign the proper collection for software assignments
    ForEach ($Rule in $PilotUpdateDeviceCol.CollectionRules)
    {
        If($Rule.ResourceID -eq $ResourceQuery.ResourceId){
            WriteLog "Resource is already in Collection <$($PilotUpdateDeviceCol.Name)> nothing to do..."  
            $PilotUpdateColExists = $true
        }
    }
    if($PilotUpdateColExists){
        ...
        AddCollectionMembership -ResourceId $ResourceQuery.ResourceId -computername $DeviceName -Collection $PilotUpdateDeviceCol
    }
}
else{
    WriteLog "Removing old PilotUpdateCollection memberships. Checking <$($PilotUpdateDeviceCol.Name)> for still active memberships"
    ForEach ($Rule in $PilotUpdateDeviceCol.CollectionRules)
    {
        If($Rule.ResourceID -eq $ResourceQuery.ResourceId){
            RemoveCollectionMembership -Collection $PilotUpdateDeviceCol -Rule $Rule
        }
    }
}
